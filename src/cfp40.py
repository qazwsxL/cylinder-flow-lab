...
def run_adam(model, dataset, device, save_dir,
    ...
    for epoch in range(1, epochs_adam + 1):
        ...
        loss, logs = compute_total_loss(
            ...
            # When PT is active, pin the collocation set so u_prev shape matches
            x_f=_xf_pt if (stepper is not None and not data_only) else None,
            y_f=_yf_pt if (stepper is not None and not data_only) else None,
            t_f=_tf_pt if (stepper is not None and not data_only) else None,
            xlim=xlim,
            ylim=ylim,
            n_cfd_pde=n_cfd_pde,
